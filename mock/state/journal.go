...
func (ch suicideChange) revert(s *StateDB) {
	...
	if obj != nil {
		...
		for _, token := range ch.prevbalance {
			obj.setTokenBalance(token.TokenAddr, token.Value)
		}
...
func (ch creditsChange) revert(s *StateDB) {
	s.getStateObject(*ch.account).setCredits(ch.prev)
}

func (ch creditsChange) dirtied() *types.Address {
	return ch.account
}
...
func (ch tokenBalanceChange) dirtied() *types.Address {
	return ch.account
}
